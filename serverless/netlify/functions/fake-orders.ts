...
const handler: Handler = async (event, context) => {
    ...
    const {amount:amountRaw = '1', recent:recentRaw = '0'} = queryStringParameters
    ...
    for (let i = 0; i < amount; i++) {
        ...
        const newOrder = await api.CreateFakeOrder(
            fakeData, {
                'x-hasura-admin-secret': config.hasuraAdminSecret
            })

        const firstGroupItem = menuItems.firstGroup[faker.datatype.number({max: firstGroupLength - 1})].id
        const secondGroupItem = menuItems.secondGroup[faker.datatype.number({max: secondGroupLength - 1})].id
